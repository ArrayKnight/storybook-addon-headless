...
A partial [Apollo Client config](https://www.apollographql.com/docs/react/api/core/ApolloClient/#required-fields) can be passed in as a base config. The most common use case would be to establish a `uri` so that all stories can simply define a query without the need for an additional config.
